...
$(document).ready(function () {
  ...
  if ($('#yandex-map').length) {
    var placemarkForBack = function placemarkForBack(coordsArray) {
      // console.log(coordsArray);
      if (coordsArray.length > 0) {
        placemarkToBack = coordsArray[0] + ";" + coordsArray[1];
        $('#coords').val(placemarkToBack); // console.log(placemarkToBack);
      }
    };
    ...
    var init = function init() {
      ...
      placemark = new ymaps.Placemark([55.76, 37.64], {
        ...
        draggable: true //   openEmptyBalloon: true
      ...
      myMap.geoObjects.add(circle); // Определяем адрес по координатам (обратное геокодирование).
      ...
      function getAddress(coords) {
        ymaps.geocode(coords).then(function (res) {
          ...
          placemark.properties.set({
            ...
            iconContent: [// Название населенного пункта или вышестоящее административно-территориальное образование.
            ...
            res.geoObjects.get(0) ? res.geoObjects.get(0).properties.get('name') : 'Не удалось определить адрес.'].filter(Boolean).join(', '),
